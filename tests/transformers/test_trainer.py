...
class TestTrainer(unittest.TestCase):
    ...
    def test_train(self, mock_set_seed, mock_scheduler_fn):
        ...
        mock_train_loader = [(torch.randn(2, 5), torch.randn(2, 2)) for _ in range(2)]
        mock_eval_loader = [(torch.randn(2, 5), torch.randn(2, 2)) for _ in range(1)]
    ...
    def test_evaluate(self):
        ...
        mock_eval_dataloader = [
            (torch.randn(2, 5), torch.randn(2, 2)) for _ in range(2)
        ]
        mock_progress_bar = MagicMock()
        self.trainer.accelerator.get_progress_bar.return_value = mock_progress_bar
