...
class MockDataset:
    def get_dataframe(self, tables):
        ...
        df = pd.DataFrame({
            ...
            "age": [65, 55, 70, 45, 60],
            "gender": ["M", "F", "M", "F", "M"],
            "condition": ["diabetes", "hypertension", "diabetes", "cancer", "hypertension"]
        })
        return df
...
def test_meds_parser_with_mock(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert abs(split_counts["train"] - 0.6) < 0.2, "Train ratio not approximately correct"
        assert abs(split_counts["valid"] - 0.2) < 0.1, "Validation ratio not approximately correct"
        assert abs(split_counts["test"] - 0.2) < 0.1, "Test ratio not approximately correct"
        ...
        assert "event_types" in metadata.columns, "Metadata missing event types information"
        assert "feature_count" in metadata.columns, "Metadata missing feature count"
        assert "categorical_features" in metadata.columns, "Metadata missing categorical features"
        assert "numerical_features" in metadata.columns, "Metadata missing numerical features"
...
def test_meds_parser_with_synthetic_data():
    ...
    with tempfile.TemporaryDirectory() as data_dir, tempfile.TemporaryDirectory() as output_dir:
        ...
        assert all(x in [0, 1] for x in df["modality"].iloc[0]), "Modality should contain only 0s and 1s"
        
        assert isinstance(df["numerics"].iloc[0], list), "Numerics should be a list"
        assert all(isinstance(x, (int, float)) for x in df["numerics"].iloc[0]), "Numerics should contain numeric values"
        
        # Check events format
        assert isinstance(df["events"].iloc[0], list), "Events should be a list"
        assert all(x in [0, 1] for x in df["events"].iloc[0]), "Events should contain only 0s and 1s"
        
        # Check durations format
        assert isinstance(df["durations"].iloc[0], list), "Durations should be a list"
        assert all(isinstance(x, (int, float)) for x in df["durations"].iloc[0]), "Durations should contain numeric values"
        
        # Check data shape requirements
        assert len(df["modality"].iloc[0]) == len(df["numerics"].iloc[0]), "Modality and numerics should have the same length"
        assert len(df["events"].iloc[0]) == len(df["durations"].iloc[0]), "Events and durations should have the same length"
        
        # Check split distribution
        assert set(df["split"].unique()) == {"train", "valid", "test"}, "Missing data splits"
