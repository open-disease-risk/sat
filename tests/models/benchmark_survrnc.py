    ...
    for batch_size in batch_sizes:
        for num_events in num_events_list:
            ...
            try:
                ...
                # Print results
                print(
                    f"  MultiEventRankingLoss   - Forward: {multi_forward:.4f} ms, Backward: {multi_backward:.4f} ms"
                )
                print(
                    f"  SampleRankingLoss       - Forward: {sample_forward:.4f} ms, Backward: {sample_backward:.4f} ms"
                )
                print(
                    f"  SampleListMLELoss       - Forward: {sample_list_mle_forward:.4f} ms, Backward: {sample_list_mle_backward:.4f} ms"
                )
                print(
                    f"  SurvRNCLoss             - Forward: {survrnc_forward:.4f} ms, Backward: {survrnc_backward:.4f} ms"
                )
                print("")
        ...
        data.append(
            {
                ...
                "total_time_ms": results["survrnc_forward"][i]
                + results["survrnc_backward"][i],
            }
        )
...
def generate_summary_markdown(csv_path):
    ...
    for batch_size, num_events in configs:
        ...
        config_df = df[
            (df["batch_size"] == batch_size) & (df["num_events"] == num_events)
        ]

        # Get reference time (SampleRankingLoss)
        reference = config_df[config_df["method"] == "SampleRankingLoss"][
            "total_time_ms"
        ].values[0]
        ...
        for method in config_df["method"].unique():
            ...
            method_time = config_df[config_df["method"] == method][
                "total_time_ms"
            ].values[0]
    ...
    all_sample_list = [
        values.get("SampleListMLELoss", 0) for values in summary.values()
    ]
    ...
    best_method = (
        "MultiEventRankingLoss"
        if avg_multi > max(avg_sample_list, avg_survrnc)
        else ("SampleListMLELoss" if avg_sample_list > avg_survrnc else "SurvRNCLoss")
    )
...
def plot_benchmark_results(results, save_path=None):
    ...
    plt.bar(
        positions - 1.5 * width,
        results["multievent_forward"],
        width,
        label="MultiEventRankingLoss",
    )
    plt.bar(
        positions - 0.5 * width,
        results["sample_forward"],
        width,
    ...
    plt.bar(
        ...
        width,
        label="SampleListMLELoss",
    )
    plt.bar(
        positions + 1.5 * width, results["survrnc_forward"], width, label="SurvRNCLoss"
    )
    ...
    plt.bar(
        positions + 1.5 * width, results["survrnc_backward"], width, label="SurvRNCLoss"
    )
