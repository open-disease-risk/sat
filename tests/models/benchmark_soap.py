    ...
    for batch_size in batch_sizes:
        for num_events in num_events_list:
            ...
            try:
                ...
                predictions, targets = create_fake_data(
                ...
                for name, loss_fn in loss_instances.items():
                    ...
                    fresh_survival_base = (
                        1
                        - torch.cumsum(
                            torch.nn.functional.softplus(fresh_hazard), dim=2
                        )
                        / num_cuts
                    )
                    ...
                    if "soap" in name:
                        try:
                            ...
                            test_refs = torch.zeros(
                                small_batch, 4 * num_events, requires_grad=False
                            )
                            ...
                            test_hazard = torch.rand(
                                small_batch, num_events, num_cuts, requires_grad=True
                            )
                            ones = torch.ones(small_batch, num_events, 1)
                            test_surv_base = (
                                1
                                - torch.cumsum(torch.sigmoid(test_hazard), dim=2)
                                / num_cuts
                            )
                ...
                for name in loss_names:
                    ...
                    print(
                        f"  {name:20} - Forward: {forward:.4f} ms, Backward: {backward:.4f} ms, Total: {total:.4f} ms"
                    )
                ...
                if base_time > 0:
                    for name in loss_names:
                        if name != "sample_ranking":
                            speedup = (
                                base_time / results[f"{name}_total"][-1]
                                if results[f"{name}_total"][-1] > 0
                                else 0
                            )
...
def generate_summary_markdown(csv_path):
    ...
    avg_speedups = {
        method: df[df["method"] == method]["speedup"].mean() for method in methods
    }
    max_speedups = {
        method: df[df["method"] == method]["speedup"].max() for method in methods
    }
    ...
    for batch_size in df["batch_size"].unique():
        ...
        methods_in_batch = [
            m for m in batch_df["method"].unique() if m != "sample_ranking"
        ]
        best_method = max(
            methods_in_batch,
            key=lambda m: batch_df[batch_df["method"] == m]["speedup"].mean(),
        )
    ...
    markdown += (
        f"offers the best performance with {small_batch_speedup:.2f}x speedup.\n\n"
    )
    ...
    markdown += (
        f"offers the best performance with {large_batch_speedup:.2f}x speedup.\n\n"
    )
...
def plot_benchmark_results(results, save_path=None):
    ...
    fig, axes = plt.subplots(
        len(num_events_set), 2, figsize=(16, 6 * len(num_events_set))
    )
