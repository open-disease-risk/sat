...
class SyntheticOmopGenerator:
    ...
    output_path: str = "synthetic_omop.parquet"
    seed: Optional[int] = 42
    censoring_time: int = 1095  # Default 3 years (max follow-up in days)
    
    # Data definitions
    categorical_covariates: List[Dict[str, Any]] = field(default_factory=lambda: CATEGORICAL_COVARIATES)
    numerical_covariates: List[Dict[str, Any]] = field(default_factory=lambda: NUMERICAL_COVARIATES)
    ...
    _df_patient_data: Optional[pd.DataFrame] = field(default=None, init=False, repr=False)
    _patient_birth_dates: Dict[int, datetime] = field(default_factory=dict, init=False, repr=False)
    _patient_enrollment_dates: Dict[int, datetime] = field(default_factory=dict, init=False, repr=False)
    _patient_derived_features: Dict[int, Dict[str, Any]] = field(default_factory=dict, init=False, repr=False)
    _patient_risk_scores: Dict[int, Dict[str, float]] = field(default_factory=dict, init=False, repr=False)
    _omop_events: List[Dict[str, Any]] = field(default_factory=list, init=False, repr=False)
    ...
    def generate_patient_data(self):
        ...
        for feature in self.numerical_covariates:
            if not feature["time_dependent"]:
                if feature["distribution"] == "normal":
                    values = np.random.normal(feature["mean"], feature["std"], self.num_patients)
                    ...
                    values = np.random.uniform(feature["min"], feature["max"], self.num_patients)
        ...
        return self._df_patient_data, self._patient_birth_dates, self._patient_enrollment_dates, self._patient_derived_features
    ...
    def generate_event_times(self):
        ...
        for patient_id, enrollment_date in self._patient_enrollment_dates.items():
            ...
            hospitalization_risk = self._patient_risk_scores[patient_id]["HOSPITALIZATION"]
            ...
            for feature in self.numerical_covariates:
                if feature["time_dependent"]:
                    ...
                    for _ in range(num_measurements):
                        ...
                        if "random_health_factor" not in self._patient_derived_features[patient_id]:
                            self._patient_derived_features[patient_id]["random_health_factor"] = patient_factor
    ...
    def save_omop_to_parquet(self, metadata: Dict[str, Any] = None):
        ...
        """
        logger.info(f"Saving OMOP format data to {self.output_path}")
        
        # Get base directory and dataset name
        dir_name = os.path.dirname(self.output_path)
        dir_name = dir_name or "."  # Use current directory if no directory specified
        base_name = os.path.basename(self.output_path).split(".")[0]
        ...
        subjects_splits = pd.DataFrame({
            ...
            "split": ["train"] * len(unique_patients)
        })
        ...
        logger.info(f"Saved subjects_splits.parquet with {len(subjects_splits)} subjects")
    ...
    def log_statistics(self):
        ...
        for code, count in sorted(event_types.items(), key=lambda x: x[1], reverse=True):
            n_patients = len(set(e["patient_id"] for e in self._omop_events if e["code"] == code))
            if n_patients > 0:
    ...
    def __call__(self):
        ...
        """
        logger.info(f"Generating synthetic OMOP dataset with {self.num_patients} patients")
        
        # 1. Generate patient data and static features
        self.generate_patient_data()
        logger.info(f"Generated patient base data with {len(self._df_patient_data)} records")
        
        # 2. Generate risk scores for each event type based on patient characteristics
        self.generate_event_risk_scores()
        logger.info(f"Generated risk scores for {len(self._patient_risk_scores)} patients")
        ...
        dataset_metadata = {
            "dataset_name": os.path.basename(self.output_path).split('.')[0],
            "description": "Synthetic OMOP-format dataset for survival analysis",
            "num_patients": self.num_patients,
...
def generate_omop_data(config):
    ...
    generator = SyntheticOmopGenerator(
        ...
        output_path=getattr(config, "output_path", "synthetic_omop"),
...
def main():
    ...
    parser.add_argument(
        "--censoring_time", type=int, default=1095, help="Maximum follow-up time in days"
    )
