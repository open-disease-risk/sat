...
class RankingLoss(Loss):
    ...
    def ranking_loss(
        ...
        I = events.bool()
        I_censored = ~I  # censored indicator (n x e) - more efficient than events.to(bool) and then negating
        ...
        t0Index = torch.sum(indexSmaller, dim=2, keepdim=False) - 1  # left boundary of time interval (n x e)
        ...
        SatT0 = torch.gather(survival, 2, t0Index_expanded)  # survival at T0 (n x e x e)
        ...
        if positive_mask.any():
            ...
            log_diff = (torch.log(SatT0[positive_mask] + eps) - 
                        torch.log(SatT1[positive_mask] + eps))
            hstar[positive_mask] = log_diff / dT[positive_mask]  # solve for hazard given the survival at T0 and T1 (n x e x e)
        ...
        SatTMinus = SatT0 * torch.exp(-(TMinus - T0) * hstar)  # solve for survival at time t-epsilon (n x e x e)
        ...
        diag_S2 = torch.diagonal(SatTMinus, dim1=-2, dim2=-1).unsqueeze(2).expand(n, e, e)
        ...
        dS2 = SatTMinus - diag_S2_T  # dS_{ij} = S_{i}(T_{j}-1) - S_{j}(T_{j}-1) (n x e x e)
        dS3 = SatT - diag_S.transpose(1, 2)  # dS_{ij} = S_{i}(T_{j}) - S_{j}(T_{j}) (n x e x e)
        ...
        dur_diff = F.relu(torch.sign(durations_expanded - durations.unsqueeze(2).expand(n, e, e)))
        ...
        if self.l2_type == "uncensored":
            ...
            if not_event_indicators.any():
                ...
                weights = torch.tensor(
                    1.0 - self.kms[event_type].predict(censor_np),
                    device=device
                )
                best_guesses = torch.tensor(
                    self.kms[event_type].best_guess(censor_np),
                    device=device
                )
    ...
    def ce(self, predictions, references, event_type) -> torch.Tensor:
        ...
        if self.l_type == "uncensored":
            ...
            if relevant_for_classification.any():
                ...
                preds = predictions[:, event_type].index_select(0, relevant_indices).squeeze(-1)
                targets = targets.index_select(0, relevant_indices)
                
                # Compute loss directly with weight factor
                loss = self.loss_func(preds, targets).mean() * self.weights[event_type + 1]
            else:
                # Handle empty case
            ...
            if not_event_indicator.any():
                ...
                if event_indicator.any():
                    event_occurred[event_indicator] = durations[event_indicator] <= self.event_time_thr
                
                # Only process if we have censored events
                if not_event_indicator.any():
                    event_occurred[not_event_indicator] = best_guesses <= self.event_time_thr
                ...
                if relevant_for_classification.any():
                    ...
                    not_event_mask = not_event_indicator.index_select(0, relevant_indices)
                    ...
                    if total_weight > 0:
                        weighted_multiplier = 1.0 / total_weight
                        loss = self.weights[event_type + 1] * weighted_multiplier * losses.mean()
                    else:
                        loss = torch.zeros(1, device=device)
                else:
                    loss = torch.zeros(1, device=device)
