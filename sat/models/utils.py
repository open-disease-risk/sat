...
def _detect_problematic_modules(model):
    ...
    for name, module in model.named_modules():
        ...
        for issue in ['softplus', 'index_put', 'log_softmax', 'logsoftmax', 'log_sigmoid']:
...
def compile_model(model, config=None):
    ...
    try:
            ...
            try:
                if module_name in sys.modules or importlib.util.find_spec(module_name):
                    ...
                    for attr_name in dir(mod):
                        if not attr_name.startswith('_') and callable(getattr(mod, attr_name)):
                            full_name = f"{module_name}.{attr_name}"
                            if full_name not in exclude_modules:
                                exclude_modules.append(full_name)
                                logger.debug(f"Added {full_name} to compilation exclusion list")
        ...
        try:
            ...
            exclude_modules.append('sat.evaluate.eval_modules.SurvivalAnalysisMetric.compute')
            exclude_modules.append('sat.evaluate.eval_modules.SurvivalAnalysisMetric.compute_event')
        except ImportError:
            pass
    ...
    try:
            ...
            try:
                ...
                if is_m_series:
                    ...
                    if compile_mode is None:
                        compile_mode = m_series_config.get('compile_mode', 'reduce-overhead')
                    ...
                    if m_series_spec_opts:
                        static_memory_planning = m_series_spec_opts.get('static_memory_planning', static_memory_planning)
                        disable_fuser = m_series_spec_opts.get('disable_fuser', disable_fuser)
        ...
        if cache_enabled and hasattr(torch, '_dynamo'):
            if hasattr(torch._dynamo, 'config'):
                if cache_size_limit is not None and hasattr(torch._dynamo.config, 'cache_size_limit'):
                    torch._dynamo.config.cache_size_limit = cache_size_limit
                torch._dynamo.config.suppress_errors = True
                
                if cache_path is not None and hasattr(torch._dynamo.config, 'cache_dir_path'):
                ...
                if hasattr(torch, 'compiler') and hasattr(torch.compiler, 'disable'):
                    ...
                    for module_name in exclude_modules:
                        try:
                            ...
                            logger.info(f"Disabled compilation for module: {module_name}")
                        except Exception as e:
                            logger.warning(f"Could not disable module {module_name}: {e}")
                    ...
                    if exclude_patterns:
                        ...
                        logger.info(f"Pattern-based exclusions will be applied: {exclude_patterns}")
            except Exception as e:
        ...
        if is_m_series:
            logger.info(f"Using M-series optimized settings (opt_level={opt_level}, dynamic_shapes={dynamic_shapes})")
        ...
        try:
            ...
            if explain and hasattr(torch, '_dynamo') and hasattr(torch._dynamo, 'explain'):
                explanation = torch._dynamo.explain(model)
                logger.info(f"Compilation explanation: {explanation}")
        except Exception as e:
            logger.warning(f"Initial compilation failed: {e}, trying with reduced settings")
