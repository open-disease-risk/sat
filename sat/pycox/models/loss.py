...
def nll_pc_hazard_loss(
    ...
    safe_interval_frac = torch.nan_to_num(interval_frac, nan=0.0, posinf=1.0, neginf=0.0)
    safe_interval_frac = torch.clamp(safe_interval_frac, min=0.0, max=1.0)  # Ensure it's between 0 and 1
    ...
    if invalid_mask.any():
        ...
        print(f"Warning: Found {invalid_mask.sum().item()} invalid loss values. Replacing with zeros.")
