        ...
        for i in range(self.num_events):
            ...
            if mask.sum() > 0:
                ...
                event_hazard_at_t = torch.gather(
                    event_hazards, 1, time_idx.unsqueeze(1)
                ).squeeze(1)
                ...
                event_survival_before_t = torch.gather(
                    event_survival[:, i, :], 1, time_idx.unsqueeze(1)
                ).squeeze(1)
                ...
                if self.weights is not None:
                    event_nll = (
                        event_nll * self.weights[i + 1]
                    )  # +1 to skip first weight
        ...
        if censored_mask.sum() > 0:
            ...
            for i in range(self.num_events):
                ...
                surv_i_at_censor = torch.gather(
                    censored_survival[:, i, :], 1, (censored_times + 1).unsqueeze(1)
                ).squeeze(1)
            ...
            if self.weights is not None:
                censor_nll = (
                    censor_nll * self.weights[0]
                )  # First weight is for censored
        ...
        for event_type in range(self.num_events):
            ...
            for i, time_i in enumerate(event_times):
                ...
                survival_idx = torch.min(
                    time_bin_idx + 1,
                    torch.tensor(survival.size(2) - 1, device=time_bin_idx.device),
                )
