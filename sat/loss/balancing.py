...
class LossBalancer(nn.Module):
    ...
    def forward(self, losses: List[torch.Tensor], iteration: Optional[int] = None) -> torch.Tensor:
        """
        ...
        **kwargs
        ...
        if isinstance(strategy, str):
            try:
                ...
                logger.warning(f"Unknown balancing strategy: {strategy}. Using fixed weighting.")
        ...
        with torch.no_grad():
            for i, loss in enumerate(losses):
                if iteration is None or iteration > 0:  # Skip first iteration
                    ...
                    self.loss_scales[i] = self.alpha * self.loss_scales[i] + (1 - self.alpha) * curr_scale
    ...
    def forward(self, losses: List[torch.Tensor], iteration: Optional[int] = None) -> torch.Tensor:
        ...
        for i, loss in enumerate(losses):
            ...
            if hasattr(loss, 'grad_fn') and loss.grad_fn is not None:
                ...
                if iteration is None or iteration > 0:  # Skip first iteration
                    self.grad_norms[i] = self.alpha * self.grad_norms[i] + (1 - self.alpha) * curr_grad_norm
        ...
        with torch.no_grad():
            ...
            for i, loss in enumerate(losses):
                ...
                if self.iteration_counter > 0:
                    self.current_losses[i] = self.alpha * self.current_losses[i] + (1 - self.alpha) * curr_loss
                else:
            ...
            if self.iteration_counter >= self.window_size:
                ...
                self.coeffs = torch.clamp(self.coeffs + weight_adjustments, min=0.1, max=10.0)
                
                # Normalize weights to sum to num_losses
                self.coeffs = self.coeffs * (self.num_losses / (self.coeffs.sum() + self.eps))
