...
class SOAPLoss(RankingLoss):
    ...
    def sample_pairs(
        ...
        if max_pairs is None:
            if self.num_pairs is None:
                ...
                if batch_size <= 32:
                    ...
                    max_pairs = int(batch_size * 2 * torch.log2(torch.tensor(batch_size, device=device)))
                else:
                    # Large batches use aggressive logarithmic scaling
                    max_pairs = int(batch_size * torch.log2(torch.tensor(batch_size, device=device)))
            else:
        ...
        if self.sampling_strategy == "uniform":
            ...
            if batch_size <= 128:
                ...
                row_idx, col_idx = torch.triu_indices(batch_size, batch_size, offset=1, device=device)
                ...
                j_indices = torch.randint(0, batch_size - 1, (max_pairs,), device=device)
            ...
            if subset_size < len(valid_indices):
                ...
                subset_idx = torch.randperm(len(valid_indices), device=device)[:subset_size]
            ...
            anchor_idx, compare_idx = torch.triu_indices(n_valid, n_valid, offset=1, device=device)
            
            # Limit to manageable size for memory efficiency
            if anchor_idx.shape[0] > n_valid_pairs:
                select_idx = torch.randperm(anchor_idx.shape[0], device=device)[:n_valid_pairs]
            ...
            if importance_scores:
                ...
                if len(combined_importance) > 0:
                    ...
                    if torch.allclose(probs.sum(), torch.zeros(1, device=device)):
                        selected_indices = torch.randint(0, len(anchor_indices), (num_to_sample,), device=device)
                    else:
            ...
            weighted_durations = (durations * events).sum(dim=1) / torch.clamp(event_count.squeeze(), min=1)
            ...
            if len(valid_samples) > 10:
                ...
                for i in range(num_buckets):
                    ...
                    if len(bucket_samples) > 1:
                        ...
                        for _ in range(min(5, len(bucket_samples))):
                            pair_indices = torch.randint(0, len(bucket_samples), (2,), device=device)
                            # Ensure we don't compare a sample to itself
                            while pair_indices[0] == pair_indices[1] and len(bucket_samples) > 1:
                                pair_indices[1] = torch.randint(0, len(bucket_samples), (1,), device=device)
                            
                            hard_pairs.append([bucket_samples[pair_indices[0]], bucket_samples[pair_indices[1]]])
                ...
                if hard_pairs:
                    ...
                    if len(uniform_result) > 0:
                        ...
                        # Shuffle
                        shuffle_idx = torch.randperm(len(combined), device=device)[:max_pairs]
    ...
    def compute_soap_loss(
        ...
        for event_idx in range(num_events):
            ...
            both_events_mask = anchor_events[:, event_idx] & compare_events[:, event_idx]
            ...
            if both_events_mask.any():
                ...
                if valid_diff_mask.any():
                    ...
                    s_anchor = self.interpolate_survival(
                        survival[a_idx, event_idx], 
                        t_anchor_valid,
                        device
                    )
                    s_compare = self.interpolate_survival(
                        survival[c_idx, event_idx],
                        t_compare_valid,
                        device
                    )
                    ...
                    loss_accumulator = loss_accumulator + torch.sum(pair_losses * event_weight)
                    valid_count_accumulator += len(final_indices)
            
            # --- CASE 2: Anchor has event, comparison is censored ---
            anchor_event_only = anchor_events[:, event_idx] & ~compare_events[:, event_idx]
            ...
            compare_event_only = ~anchor_events[:, event_idx] & compare_events[:, event_idx]
    ...
    def interpolate_survival(
        ...
        if time_points.dim() == 1:
            if time_points.shape[0] != survival_curve.shape[0]:
                ...
                time_points = time_points.unsqueeze(0).expand(survival_curve.shape[0], -1)
            else:
        ...
        is_greater = (time_expanded >= cuts_expanded).to(torch.float32)  # [batch_size, num_points, num_cuts]
        
        # Get the rightmost index where time >= cut
        # Sum across cuts dimension and subtract 1
        indices = torch.sum(is_greater, dim=2, keepdim=True) - 1  # [batch_size, num_points, 1]
