...
class SurvRNCLoss(RankingLoss):
    ...
    def compute_time_similarity_matrix(self, durations, event_indicators):
        ...
        time_diff = torch.abs(durations.unsqueeze(1) - durations.unsqueeze(0))  # [batch, batch, num_events]
        ...
        event_mask = event_indicators.unsqueeze(1) * event_indicators.unsqueeze(0)  # [batch, batch, num_events]
    ...
    def interpolate_survival_batch(self, survival, durations, event_indicators):
        ...
        flat_batch_indices = torch.arange(batch_size, device=device).repeat_interleave(num_events)
        ...
        if interp_mask.any():
            ...
            interpolated_survival[interp_mask] = s0[interp_mask] * torch.exp(-(interp_durations - interp_t0) * h_star)
        ...
        survival_at_times[valid_batch_indices, valid_event_indices] = interpolated_survival
    ...
    def compute_risk_similarity_matrix(self, survival, durations, event_indicators):
        ...
        survival_at_times = self.interpolate_survival_batch(survival, durations, event_indicators)
        ...
        risk_diff = torch.abs(risk_at_times.unsqueeze(1) - risk_at_times.unsqueeze(0))  # [batch, batch, num_events]
    ...
    def compute_contrastive_loss(self, similarity_matrix, time_similarity_matrix):
        ...
        if self.use_hard_mining and batch_size > 128:
            ...
            return self._compute_hard_mining_loss(scaled_similarity, time_similarity_matrix)
        ...
        for i in range(batch_size):
            ...
            pos_mask = (time_sim_i > pos_threshold) & (torch.arange(batch_size, device=device) != i)
            neg_mask = (time_sim_i < neg_threshold) & (torch.arange(batch_size, device=device) != i)
