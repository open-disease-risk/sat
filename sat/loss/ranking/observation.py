...
class ObservationEventRankingLoss(RankingLoss):
    ...
    def __init__(
        ...
        if num_events < 2:
            raise ValueError("ObservationEventRankingLoss requires at least 2 event types")
    ...
    def forward(self, predictions: SAOutput, references: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(batch_size):
            ...
            risk_idx = torch.min(
                time_bin_idx + 1, 
                torch.tensor(risk.size(2) - 1, device=device)
            )
            ...
            for j in range(self.num_events):
                ...
                if self.weights is not None:
                    ...
                    weight = (self.weights[actual_event_idx + 1] + self.weights[j + 1]) / 2
