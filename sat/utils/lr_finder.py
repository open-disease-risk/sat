...
class LRFinder:
    ...
    def _compute_lr_step(self, step):
        ...
        if self.num_steps <= 1:
            return self.end_lr
        return self.start_lr * (self.end_lr / self.start_lr) ** (step / (self.num_steps - 1))
        ...
        for i, loss in enumerate(self.losses):
            if i == 0:
                ...
                smoothed_losses.append(
                    self.smooth_factor * loss + (1 - self.smooth_factor) * smoothed_losses[-1]
                )
    ...
    def find(self, save_plot_path: Optional[str] = None):
        ...
        # Start finder
        logger.info(f"Starting learning rate finder from {self.start_lr} to {self.end_lr}")
        ...
        try:
            for batch_idx, batch in enumerate(dataloader):
                ...
                if torch.isnan(loss) or torch.isinf(loss):
                    logger.warning(f"Invalid loss detected at lr={lr}. Stopping LR search.")
                    break
                ...
                # Log progress
                if (batch_idx + 1) % 5 == 0:
                    logger.info(f"Step {step+1}/{self.num_steps}: lr={lr:.8f}, loss={loss.item():.4f}")
        ...
        if len(self.lrs) > 10:
            ...
            # Compute derivatives
            derivatives = [(smoothed_losses[i+1] - smoothed_losses[i]) / (self.lrs[i+1] - self.lrs[i]) 
            ...
            for i, derivative in enumerate(derivatives):
                if derivative < 0 and (min_derivative_idx is None or derivative < derivatives[min_derivative_idx]):
    ...
    def _create_plot(self, save_path):
        ...
        plt.ylabel("Loss")
        plt.axvline(x=self.best_lr, color='r', linestyle='--', alpha=0.5, 
